...
export async function researchTrend(topic, options = {}) {
  ...
  const [redditResults, hnResults, ytResults, polyResults, ghResults, bskyResults] =
    await Promise.all([
  ...
  const totalResults =
    ...
    polyResults.length + ghResults.length + bskyResults.length;
